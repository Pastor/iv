...
struct _Device
{
    ...
    volatile int32_t   rx_index;
    uint8_t            tx[256];
    volatile uint32_t  tx_index;
    volatile uint32_t  tx_len;
    volatile enum _Firmware fw;
    volatile int       loop;
};
...
typedef void(*receive_callback)(int fw, struct _Result result[100], int count, void *user);
...
int  device_receive(receive_callback cb, void *userdata, struct _Device *dev);

void device_fw_read60(receive_callback cb, void *userdata, struct _Device * dev, int *ret);
